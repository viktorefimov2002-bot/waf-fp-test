...
func Load(path string) (runner.Config, error) {
	...
	known := map[string]func(string) error{
		...
		"payload_variants.transforms":     func(v string) error { p, e := payloadvariant.Parse(parseList(v)); cfg.Variants = p; return e },
