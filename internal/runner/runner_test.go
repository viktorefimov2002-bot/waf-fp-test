...
func TestBuildRequestMatrix(t *testing.T) {
	tests := []struct {
		...
		{PlacementQuery, http.MethodGet, "", func(t *testing.T, r *http.Request) {
			if got := r.URL.Query().Get("fp_param"); got != "select from catalog" { t.Fatalf("query=%q", got) }
		}},
		{PlacementForm, http.MethodPost, "application/x-www-form-urlencoded", func(t *testing.T, r *http.Request) {
			body, _ := io.ReadAll(r.Body)
			if !strings.Contains(string(body), "fp_param=") { t.Fatalf("body=%q", body) }
		}},
		{PlacementJSON, http.MethodPost, "application/json", func(t *testing.T, r *http.Request) {
			body, _ := io.ReadAll(r.Body)
			if !strings.Contains(string(body), `"fp_param":"select from catalog"`) { t.Fatalf("body=%q", body) }
		}},
		{PlacementHeader, http.MethodGet, "", func(t *testing.T, r *http.Request) {
			if got := r.Header.Get("X-WAF-FP-Value"); got != "select from catalog" { t.Fatalf("header=%q", got) }
		}},
		{PlacementCookie, http.MethodGet, "", func(t *testing.T, r *http.Request) {
			if len(r.Cookies()) != 1 || r.Cookies()[0].Name != "fp_param" { t.Fatalf("cookies=%v", r.Cookies()) }
		}},
		{PlacementPath, http.MethodGet, "", func(t *testing.T, r *http.Request) {
			if !strings.Contains(r.URL.EscapedPath(), "select%20from%20catalog") { t.Fatalf("path=%q", r.URL.EscapedPath()) }
		}},
	}
