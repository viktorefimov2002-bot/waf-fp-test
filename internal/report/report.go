...
func WriteSummary(resultsPath, outputPath string) (Summary, error) {
	...
	for _, r := range results {
		...
		s.ByPlacement[string(r.Placement)]++
	}
	...
	b.WriteString("\n## Actionable results\n\n| Verdict | Placement | Method | Payload |\n|---|---|---|---|\n")
	for _, r := range results {
		if r.Verdict == "CONFIRMED_FP" || r.Verdict == "BLOCKED_BENIGN_CANDIDATE" || r.Verdict == "FLAKY_FP" || r.Verdict == "RESPONSE_DIFFERENCE" {
			fmt.Fprintf(&b, "| %s | %s | %s | %s |\n", r.Verdict, r.Placement, r.Method, escape(r.Payload))
		}
	}
...
func WriteComparison(baselinePath, currentPath, outputPath string) (Comparison, error) {
	...
	for key, old := range bmap {
		if isFP(old.Verdict) {
			if cur, ok := cmap[key]; !ok || !isFP(cur.Verdict) {
				c.FixedFP++
	...
	b.WriteString("| Change | Placement | Method | Payload |\n|---|---|---|---|\n")
	for _, r := range rows {
		b.WriteString(r)
	}
	if err := os.WriteFile(outputPath, []byte(b.String()), 0o644); err != nil {
...
func key(r runner.Result) string { return strings.Join([]string{r.Mode, string(r.Placement), r.Method, r.Payload}, "\x00") }
func isFP(v string) bool          { return v == "CONFIRMED_FP" || v == "BLOCKED_BENIGN_CANDIDATE" }
func row(change string, r runner.Result) string {
	return fmt.Sprintf("| %s | %s | %s | %s |\n", change, r.Placement, r.Method, escape(r.Payload))
}
